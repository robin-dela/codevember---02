...
void main() {
  ...
  float distortion = sin(vUv.y * 1.0 + time * 1.0) * 1.0;

  float y = mod(gl_FragCoord.y * mapy.r + (distortion * 150.0), 20.) < 0.8 ? 1. : 0.;
	float x = mod(gl_FragCoord.x * mapy.r + (distortion * 150.0), 20.) < 6. ? 1. : 0.;
